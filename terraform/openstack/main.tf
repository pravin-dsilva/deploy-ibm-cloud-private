...
resource "openstack_compute_instance_v2" "icp-worker-vm" {
    ...
    provisioner "local-exec" {
        command = "if nc -z $MASTER_IP 22; then ssh -o 'StrictHostKeyChecking no' -i $KEY_FILE $USER@$MASTER_IP 'if [[ -f /tmp/icp_worker_scaler.sh ]]; then chmod a+x /tmp/icp_worker_scaler.sh; /tmp/icp_worker_scaler.sh a ${var.icp_edition} ${self.network.0.fixed_ip_v4}; fi'; fi"
        environment {
